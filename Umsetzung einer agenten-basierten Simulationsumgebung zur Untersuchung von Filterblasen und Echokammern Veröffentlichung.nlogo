...
;;creation of the models world
to setup
  clear-all
  set starting-seed new-seed ;;starting-seed is a randomly generated seed. If you want to use a specific seed change "new-seed" to the desired value
  output-print word "Seed for this run: " starting-seed
  random-seed starting-seed
  create-guys agentnumber [konstruktorGuy]  ;;The number of generated people is eqaual to the value of the agentnumber parameter. Every person start out by calling their constructor ("konstruktorGuy")
  create-information initialInformation * agentnumber [konstruktorInfo]  ;;Every single information calls their constructor (konstruktorInfo) when initialised. initialInformation * agentnumber is used to determine the amount of initial information within the world
...
;;The "go"-function is the main function telling agents what to do
to go
  while[ticks < 8760][ ;;This loop determines what is happening during each tick
    ask friendships [hide-link] ;; hide-link is generally used to hide links wihtin the model to keep the view clear
    ask sourcefriends [hide-link]
    create-information 1 [konstruktorInfo] ;;a new information is created every tick at a random position
    ask guys [
  ...
  ;;once the model completed every tick it will carry out a clusteranalysis
  ask guys[
    initializeCluster
  ]
  ask guys[
  ...
  ;;Initialising and drawing the plot used for the elbow criterium
...
;;"constructor" of the "person"-agents
to konstruktorGuy
  setxy random-xcor random-ycor ;;each person starts with randomly generated coordinates
  set color 125
  set agentmemory memory ;;defines a persons memory. The value is equal to the switch in the UI
  set numberoffriends potentialFriends ;;defines the amount of friends each person can have based on the switch in the UI
  set latitude acceptanceLatitude  ;;defines the latitude of acceptance based on the switch in the UI
end

to konstruktorInfo
  setxy random-xcor random-ycor ;;each information is intialised with randomly generated coordinates
  set shape "circle"
  set color 95
end

;;each of the functions used within the model. each of these functions can be called by any agent of the person-type
to addKnowledge [newinfo]
  ;;comparing the coordinates of information and people including their latiitude of acceptance
  ifelse sourcecredibility = true[ ;;check wether source credibility is used or not. followed by another check wether the latitude of acceptance is used as an absolute value or as a function
    let exponent random 2  ;;random x generates a random number 0 <= random number < x
    let a random-float 1 * (-1 ^ exponent) ;;random-float genereates a random float between 0 and x, x is 1 in this case
    ifelse latitudeFunction = true[
      if random-float 1 < integration-probability distance newinfo[
        ifelse (count sourceknown-neighbors) < agentmemory[    ;;check to see if the persons memory is full already
          create-sourceknown-to newinfo [set informationAttitude a]
          ask knowledge [set color 65]
          setxy (mean [xcor] of sourceknown-neighbors) (mean [ycor] of sourceknown-neighbors)  ;;setxy is used in combination with mean to change a persons position based on the average coordinates of their integrated information
        ][
          ask one-of my-out-sourceknowledge [die]  ;;delete a random integrated information if a persons memory is already full
          create-sourceknown-to newinfo [set informationAttitude a]
          ask knowledge [set color 65]
          setxy (mean [xcor] of sourceknown-neighbors) (mean [ycor] of sourceknown-neighbors)
        ]
        ;;checking if an information that has just been integrated will be shared with a friend
    ...
    ifelse latitudeFunction = true[  ;;integration of information if the latitude of acceptance is used as a function
      let dist integration-probability distance newinfo  ;;calling the function of integration-probability by Geschke et al. (2019)
...
to addKnowledgeSrcCred [newinfo m source]  ;; the integration of information when sourcecredibility is considered. this function is used whent sharing information
  if count sourcefriend-neighbors > 0 and sourcefriend-neighbor? source = true[  ;;Pr checking wethere the receiver has friends and if the sender is one if those friends
    let a 0
    ifelse out-sourceknown-neighbor? newinfo = true[
      set a [informationAttitude] of out-sourceknown-to newinfo  ;;changing a to the value of the receivers attitude towards the information given it has already been integrated by them
    ][
